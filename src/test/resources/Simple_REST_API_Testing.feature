...
Background: 
	Given Simple init 
	And Set basic web application url is "http://localhost:8080/imademethink/restfulapi/account_basic/" 
	And Set basic user details are 
		| particular                      | value           |
		| email                           | Happy01@aaa.com |
		| password                        | k9aPass         |
		| first_name                      | Sheldon         |
		| last_name                       | Cooper          |
		| gender                          | m               |
...
Scenario: PUT request example 
	...
	And Set PUT api endpoint as "modify_account_profile_details" 
	And Set HEADER param request content type as "application/x-www-form-urlencoded" 
	And Set HEADER param response accept type as "application/json" 
	And Set BODY form param using basic user details 
...
Scenario: DELETE request example 
	...
	And Set DELETE api endpoint as "signout" 
	And Set HEADER param request content type as "application/json" 
	And Set HEADER param response accept type as "application/json" 
